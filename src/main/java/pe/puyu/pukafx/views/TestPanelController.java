  ...
  void onClickBtnTest(ActionEvent event) {
    Platform.runLater(() -> {
      try {
        ...
        var bytes = PrintTestService.customDesing(escpos -> {
          try {
  ...
  void onClickBtnPrint(ActionEvent event) {
    Platform.runLater(() -> {
      try {
        ...
        sweetTicketPrinter
            .setOnSuccess(() -> showMessageAreaError("La prueba no lanzo una excepcion.", "info"))
            .setOnError(error -> showMessageAreaError(error, "error"))
            .printTicket();
      } catch (Exception e) {
        showMessageAreaError(e.getMessage(), "error");
